...
// .wrangler/tmp/bundle-1OsgbW/checked-fetch.js
...
async function searchHotels(params, env2) {
  try {
    const amadeus = await getAmadeusClient(env2);
    const hotelParams = {
      cityCode: params.city.length === 3 ? params.city.toUpperCase() : params.city,
    ...
    const hotelOffers = await amadeus.get("/v2/shopping/hotel-offers", hotelParams);
    return formatHotelResults(hotelOffers.data, params);
  } catch (error) {
    console.error("Error searching hotels:", error);
    if (error.message?.includes("Invalid city code") || error.message?.includes("Parameter cityCode")) {
      throw new Error(`Invalid city code. Please use a 3-letter IATA city code (e.g., NYC for New York, LAX for Los Angeles, etc.)`);
    }
    if (error.message?.includes("Authentication failed")) {
      throw new Error("Authentication failed. Please check your Amadeus API credentials.");
    }
...
// .wrangler/tmp/bundle-1OsgbW/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-1OsgbW/middleware-loader.entry.ts
