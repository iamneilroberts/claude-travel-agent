...
// .wrangler/tmp/bundle-23uPfl/checked-fetch.js
...
// .wrangler/tmp/bundle-23uPfl/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-23uPfl/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
// node_modules/@modelcontextprotocol/sdk/dist/server/sse.js
init_checked_fetch();
init_modules_watch_stub();
...
__name(SSEServerTransport, "SSEServerTransport");
...
__name(getOrCreateServer, "getOrCreateServer");
async function handleSseRequest(request, env, ctx) {
  const server = getOrCreateServer();
  const transport = new SSEServerTransport("/", (response) => {
...
async function handleMcpRequest(request, env, ctx) {
  try {
    ...
    if (transport.onmessage) {
      await transport.onmessage({ data: JSON.stringify(jsonRpcRequest) });
    }
...
var GooglePlacesFetchClient = class {
  ...
  async findPlace(params) {
    ...
    url.searchParams.set("input", params.query);
    url.searchParams.set("inputtype", params.inputtype || "textquery");
    ...
    try {
      ...
      if (params.max_results && data.candidates) {
        data.candidates = data.candidates.slice(0, params.max_results);
      }
      return data;
    } catch (error) {
  ...
  getPlacePhotoUrl(params) {
    ...
    url.searchParams.set("photoreference", params.photo_reference);
    url.searchParams.set("key", this.apiKey);
    if (params.max_width) {
      url.searchParams.set("maxwidth", params.max_width.toString());
    }
    if (params.max_height) {
      url.searchParams.set("maxheight", params.max_height.toString());
    }
    return url.toString();
  }
};
__name(GooglePlacesFetchClient, "GooglePlacesFetchClient");
...
var findPlaceTool = {
  ...
  handler: async (request) => {
    try {
      ...
      const args = CallToolRequestSchema.parse(request).params.arguments;
...
function initializeTools(env) {
  const server = getOrCreateServer();
  setEnvironment(env);
  server.setRequestHandler(findPlaceTool);
  server.setRequestHandler(getPlaceDetailsTool);
  server.setRequestHandler(getPlacePhotoUrlTool);
}
